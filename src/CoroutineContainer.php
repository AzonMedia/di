        ...
        if (is_a($class_name, CoroutineDependencyInterface::class, TRUE) && \Swoole\Coroutine::getCid() > 0) {
            ...
            if (!isset($Context->{$class_name})) {
                if (!empty($Context->is_in_cleanup)) {
                    throw new RunTimeException(sprintf('The coroutine %s context is in dependency cleanup at the end of coroutine execution and a new coroutine dependency %s is requested.', \Swoole\Coroutine::getcid(), $id));
                }
